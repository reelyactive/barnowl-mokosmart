...
const NETWORK_STATUS_MSG_ID = 3003;         // These are from an older
const SCANNED_BLUETOOTH_DATA_MSG_ID = 3004; //   MOKO protocol
const SCAN_NOTIFICATION_MSG_ID = 3070;
const SUPPORTED_MSG_IDS = [ NETWORK_STATUS_MSG_ID,
                            SCANNED_BLUETOOTH_DATA_MSG_ID,
                            SCAN_NOTIFICATION_MSG_ID ];
...
/**
 * Decode the given advertising packet as a raddec.
 * @param {Object} data The advertising packet.
 * @param {String} receiverId The identifier of the receiver.
 * @param {Number} receiverIdType The receiver id type.
 */
function decodeAdvPacket(data, receiverId, receiverIdType) {
  let isValidReport = (typeof data.mac === 'string') &&
                      Number.isInteger(data.timestamp) &&
                      Number.isInteger(data.rssi);
  ...
  // Reconstruct packet based on availability of 'adv_data" or 'rsp_data'
  if((typeof data.adv_data === 'string') && (data.adv_data.length > 0)) {
    raddec.addPacket(reconstructPacket(data.adv_data, transmitterId));
  }
  if((typeof data.rsp_data === 'string') && (data.rsp_data.length > 0)) {
    raddec.addPacket(reconstructPacket(data.rsp_data, transmitterId));
  }
...
/**
 * Decode a scan notification message.
 * @param {Object} data The MQTT message.
 * @param {String} origin Origin of the data stream.
 * @param {Number} time The time of the data capture.
 * @param {Object} options The packet decoding options.
 */
function decodeScanNotification(data, origin, time, options) {
  if(!Array.isArray(data.data)) {
    return { raddecs: [], infrastructureMessages: [] };
  }
  ...
  data.data.forEach((advPacket) => {
    let raddec = decodeAdvPacket(advPacket, receiverId, receiverIdType);
...
function decode(data, origin, time, options) {
  ...
  switch(data.msg_id) {
    ...
    case SCAN_NOTIFICATION_MSG_ID:
      return decodeScanNotification(data, origin, time, options);
  }
}
